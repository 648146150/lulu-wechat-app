...
var TOOLS = {
  ...
  assets:{ icon:'💰',label:'资产记录',color:'assets',page:'/pages/assets/assets' },
...
function cardVal(key, rates){
  switch(key){
    ...
    case 'assets': var a=assetsStore.getSummary(rates); return {value:'¥'+a.netWorth.toFixed(0),sub:'净资产'};
...
Page({
  data: {
    ...
    cards:[], gridCols:'cols2', bookkeeping:{income:'0',expense:'0',balance:'0'}, showAssets:false
  },
  onShow:function(){
    this.tick(); this.loadWeather(); this.loadBookkeeping();
    var that=this;
    exchange.fetchRates(function(_,rates){ that.loadCards(rates); });
    if(this._t)clearInterval(this._t); this._t=setInterval(function(){that.tick();},10000);
  },
  ...
  loadCards:function(rates){
    var slots=loadSlots();
    var cards=slots.map(function(k){ var t=TOOLS[k]||TOOLS.custom; var v=cardVal(k,rates); return {key:k,icon:t.icon,label:t.label,color:t.color,page:t.page,value:v.value,sub:v.sub}; });
  ...
  toggleAssets:function(){ this.setData({showAssets:!this.data.showAssets}); }
});